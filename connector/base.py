...
class BaseConnector():
    ...
    def __init__(self, ip, port, amv_url, amv_token,
                 single_sync=False, interval=30):
        ...
        self._events = [
            'batch.start', 'batch.end', 'batch.reset', 'scan.capture',
            'scan.reject', 'scan.assign', 'scan.reassign', 'scan.unassign',
            'auth.login_user', 'auth.logout_user'
        ]
...
def base_parser():
    parser = argparse.ArgumentParser(
        description='AM-Vision Connector'
    )
    parser.add_argument(
        'ip', type=str, help='ip address to run connector on'
    )
    parser.add_argument(
        'port', type=str, help='port to run connector on'
    )
    parser.add_argument(
        'amv_url', type=str, help='url to AM-Vision API'
    )
    parser.add_argument(
        'amv_token', type=str, help='token for AM-Vision API'
    )
    parser.add_argument(
        '-s', '--single-sync', action='store_true',
